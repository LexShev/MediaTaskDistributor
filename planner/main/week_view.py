...
def week_material_list(schedules_id, worker_id, material_type, task_status, work_year, work_week, user_order,
                       order_type):
    ...
    for program_info in material_list_sql:
        ...
        # Исправление: проверяем, что day_num в пределах 0-6
        ...
        if temp_dict['Progs_program_type_id'] in (4, 8, 12):
            ...
            if repeat_index is None:
                ...
                # Добавляем эпизод к существующему сезону
                # ИСПРАВЛЕНИЕ: проверяем, что repeat_index - число
                if isinstance(repeat_index, int) and 0 <= repeat_index < len(material_list[day_num]):
                    material_list[day_num][repeat_index]['has_no_material'] = (
                            material_list[day_num][repeat_index]['has_no_material'] or
                            (temp_dict.get('Task_task_status') == 'no_material')
                    )

                    # Проверяем, что у элемента есть ключ 'episode'
                    if 'episode' in material_list[day_num][repeat_index]:
                        ...
                        # Если вдруг нет ключа 'episode', создаем фильм вместо сезона
                        program_info_dict = {
                            'Progs_program_id': temp_dict.get('Progs_program_id'),
                            'Progs_parent_id': temp_dict.get('Progs_parent_id'),
                            'Progs_name': temp_dict.get('Progs_name'),
                    ...
                    # Если repeat_index невалидный, создаем как фильм
                    program_info_dict = {
                        'Progs_program_id': temp_dict.get('Progs_program_id'),
                        'Progs_parent_id': temp_dict.get('Progs_parent_id'),
                        'Progs_name': temp_dict.get('Progs_name'),
