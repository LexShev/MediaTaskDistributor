...
def dop_search(request):
    ...
    try:
        init_dict = AdvancedSearch.objects.get(owner=user_id)
    except ObjectDoesNotExist:
        default_advanced_search = AdvancedSearch(owner=user_id, search_id=1)
        default_advanced_search.save()
        init_dict = AdvancedSearch.objects.get(owner=user_id)
        print("Новый поиск создан")

    # Сохраняем настройки при любом GET-запросе (не только POST)
    if request.method == 'GET' and request.GET:
        ...
        # Используем текущие значения из БД для initial
        form = AdvancedSearchForm(initial={
            'search_id': search_id,
            'sql_set': sql_set,
            'engineers': search_query if search_id == '3' else ''
        })

    data = {
        'search_list': query_selector(int(search_id), int(sql_set), search_query),
